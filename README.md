...
컨베이어 벨트가 자동으로 분류한 뒤, 탄소절감 에코포인트에 따라 막대사탕 1~2개를 보상으로 제공하는 인터랙티브 데모.
...
[iPad] 결과 시각화(+ 재활용 팁) → 에코포인트로 막대사탕 1~2개 결정 → 참여자에게 제공
```
...
```bash
# Pi (uv)
cd pi && uv sync && uv run pytest -q          # mock 기반, 하드웨어 불필요 (126 테스트)
