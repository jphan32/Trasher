...
| 6 | 벨트 모터 Wheeltec **MG310P20** ×2 + **드라이버**(Hiwonder 4ch I2C SA8870C, 또는 L298N류) + **I2C 레벨시프터**(hiwonder 5V↔Pi 3.3V) | 컨베이어. 전력상 2채널 |
...
> ⚠️ **배선 전 필독**: Pi 전원이 5V여도 GPIO/I2C 로직은 **3.3V·5V 비관용**. **이 Hiwonder 보드 I2C는 5V**(공식: 3.3V MCU는 레벨컨버터 안내) → **RP4B엔 양방향 레벨시프터 필수**(직결 시 Pi 손상). 상세 [`hardware.md`](hardware.md) "벨트 모터 (A)".
