...
| 6 | 벨트 모터 Wheeltec **MG310P20** ×2 + **드라이버**(Hiwonder 4ch I2C SA8870C, 또는 L298N류) | 컨베이어. 전력상 2채널. **I2C Pi 직결**(공식 키트 사례) |
| 7 | **서보·모터 전용 전원** — 서보 4.8–6V(SER0043×3 → 5V ≥3A) / 모터 7.4V(MG310P20 7.4V판, 보드 VM 5–15V·≥5A) | Pi 5V 레일과 분리 |
| 8 | 점퍼선, 공통 GND 배선 | |
| 9 | **물리 리셋 버튼 ×1**(모멘터리 NO) | 서보 홈 복귀(짧게=detach / 길게=재시팅). GPIO25↔버튼↔GND |
| 10 | **상태 OLED — YWROBOT 12864 I2C**(0.96" 128×64, SSD1306 0x3C) | Pi 동작 상태 표시(운영자용). I2C1(GPIO2/3), VCC 3.3V |
...
추가 주변장치(§6.2/§6.3): **리셋 버튼**(GPIO25↔GND, 서보 홈 복귀)과 **상태 OLED**(I2C1 GPIO2/3, 0x3C,
동작 표시). 둘 다 기본 활성·미배선 무해(graceful). 배선 상세는 [`hardware.md`](hardware.md).
...
| 리셋 버튼 | **GPIO25** | 22 | 버튼 ↔ GND(내부 풀업, 눌림=LOW) | `TRASH_RESET_BTN_PIN` |
| OLED I2C SDA | **GPIO2** | 3 | OLED SDA(I2C1, 버스 공용) | `TRASH_OLED_I2C_ADDR` |
| OLED I2C SCL | **GPIO3** | 5 | OLED SCL(I2C1, 버스 공용) | `TRASH_OLED_I2C_BUS` |
| OLED VCC | 3V3 | 1·17 | OLED 전원(~20mA) | — |
...
- **I2C1(GPIO2/3) 버스 공유**: 상태 OLED(`0x3C`)와 Hiwonder 벨트(`0x34`)는 주소가 달라 같은 SDA/SCL에 병렬로 붙일 수 있다(멀티드롭). OLED만 쓰면 GPIO23/24 gpiozero 벨트와 무충돌.
- **리셋 버튼**: 모멘터리 NO 1개, GPIO25↔버튼↔GND. 디바운스는 소프트웨어 처리(외부 RC 불필요). 짧게=detach-홈 / 길게=재시팅. 상세 [`hardware.md`](hardware.md) "리셋 버튼".
...
> **I2C 결선**: 보드 SDA/SCL/GND를 Pi GPIO2/3/GND에 **직결**(Hiwonder 공식 RPi 키트 사례 — `smbus2`·주소 `0x34`·레벨시프터 없음, Pi 내장 1.8kΩ 풀업). 보드 IIC `5V` 핀은 Pi에 연결 금지. 원칙상 Pi는 5V 비관용이라 첫 배선 시 SDA/SCL 아이들 ≤3.3V 1회 측정 권장. 상세 [`hardware.md`](hardware.md) "벨트 모터 (A)".
> `i2c` 그룹 권한은 §8 usermod에 이미 포함. 미적용 시 `i2cdetect`가 권한 오류를 낼 수 있다.

### 6.2 상태 OLED (YWROBOT 12864 I2C, SSD1306/SH1106)

Pi 동작 상태를 표시하는 I2C OLED. **기본 활성**(`TRASH_OLED=1`)이며 미배선/미활성이면 앱이 graceful
fallback(표시 없이 정상 동작). 배선·표시 내용은 [`hardware.md`](hardware.md) "상태 OLED".

```bash
# 1) I2C 활성화(§6.1과 동일 — 이미 했으면 생략). 재부팅 후 /dev/i2c-1 생성.
sudo raspi-config nonint do_i2c 0   # 또는 /boot/firmware/config.txt 에 dtparam=i2c_arm=on
sudo apt install -y i2c-tools

# 2) OLED 주소 확인(보통 0x3C, 변종 0x3D)
i2cdetect -y 1                      # 0x3C(또는 0x3D) + Hiwonder 사용 시 0x34도 함께 보임

# 3) 의존성: requirements-pi.txt 의 luma.oled(설치 절차 §10). 설치 검증:
uv run python -c "import luma.oled, PIL; print('oled lib OK')"
```

- 주소가 `0x3C`가 아니면 `TRASH_OLED_I2C_ADDR=0x3D`(§11). 제어칩이 **SH1106** 변종이면 `TRASH_OLED_CONTROLLER=sh1106`.
- I2C 활성화엔 **재부팅 필요**. install.sh가 `do_i2c`를 켜두지만 그 세션에선 `/dev/i2c-1`이 아직 없을 수 있어 OLED는 재부팅 후 표시된다(그 전까지 fallback).
- `luma.oled`는 `luma.core`·`Pillow`를 끌어온다. 큰 상태 글자는 시스템 TTF(DejaVu/Liberation)가 있으면 사용, 없으면 PIL 기본 폰트.

### 6.3 물리 리셋 버튼 (서보 홈 복귀)

GPIO25에 모멘터리 버튼 1개(↔GND). **기본 활성**(`TRASH_RESET_BTN=1`). 디바운스는 소프트웨어 처리라
외부 부품 불필요. 짧게=detach-홈 / 길게(≥0.8s)=하드스톱 재시팅. `gpio` 그룹 권한은 §8에 포함.
별도 설정 없이 동작하며, 미배선이어도(내부 풀업) 오작동 없음. 상세 [`hardware.md`](hardware.md) "리셋 버튼".
...
# GPIO 핀(§2와 일치) / 서보 구동(속도·이동·방향) / 벨트시간 — 캘리브레이션 후 조정
TRASH_GATE_PIN=17
TRASH_LEFT_PIN=27
TRASH_RIGHT_PIN=22
...
# 리셋 버튼(서보 홈) / 상태 OLED — 기본 활성. 미배선이면 자동 무시(graceful).
TRASH_RESET_BTN_PIN=25       # 버튼 GPIO(BCM). 길게=재시팅 시간은 TRASH_REHOME_SEC(1.2)
TRASH_OLED_I2C_ADDR=0x3C     # i2cdetect로 확인(변종 0x3D). SH1106이면 TRASH_OLED_CONTROLLER=sh1106
...
- [ ] (선택) 리셋 버튼 GPIO25↔GND 배선 — 짧게(detach)/길게(재시팅) 홈 복귀 동작 확인
- [ ] (선택) 상태 OLED I2C 활성화(`i2cdetect -y 1`로 `0x3C` 확인) + `luma.oled` import OK + 재부팅 후 표시
...
- [ ] 캘리브레이션(서보 속도·이동·방향/벨트시간/임계값) 완료
