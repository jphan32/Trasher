...
> ⚠️ **배선 전 필독**: Pi 전원이 5V여도 GPIO/I2C 로직은 **3.3V·5V 비관용**. 보드 I2C 풀업 전압이 사양에 없으니 **VM 인가 후 SDA/SCL 아이들 전압 측정** → 3.3V면 직결, **5V면 양방향 레벨시프터**(불확실 시 시프터로 격리). 보드 IIC `5V` 핀은 Pi에 연결 금지. 상세 [`hardware.md`](hardware.md) "벨트 모터 (A)".
