...
> ⚠️ **배선 전 필독**: Pi 전원이 5V여도 GPIO/I2C 로직은 **3.3V·5V 비관용**. 보드 SDA/SCL이 5V로 풀업되면 직결 시 Pi가 손상되니 **레벨시프터 필수**(VM 인가 후 SDA/SCL 아이들 ≤3.3V 측정 확인). 상세 [`hardware.md`](hardware.md) "벨트 모터 (A)".
