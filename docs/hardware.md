...
```
- Pi I2C는 3.3V 로직 — 보드가 3.3/5V 호환이라 직결. **RP4B는 GPIO2/3에 내장 1.8kΩ 풀업**이 있어 단일 보드·짧은 케이블이면 외부 풀업 불필요(필요 시 4.7kΩ↑ — 내장 1.8k와 병렬 과풀업 주의).
...
드라이버 VM ── 12V 외부 전원(드라이버 사양 범위 내)   드라이버 GND ── Pi GND(공통)
```
