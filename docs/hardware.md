...
| 5 | 벨트 모터 — Wheeltec MG310P20 ×2 + 드라이버(Hiwonder 4ch I2C SA8339/SA8870C, 또는 L298N류) | 컨베이어 구동(시간 기반). 전력상 2채널. **hiwonder I2C 레벨 측정 → 5V면 레벨시프터(3.3↔5V)** |
...
**(A) Hiwonder 4채널 엔코더 모터 드라이버 (SA8339/SA8870C, I2C) — 계획**

> 칩 표기 상충: 데이터시트 **SA8339** vs 공식 본문 **SA8870C**(동일 보드) — 실물 칩 마킹 확인.
> 사양(공통): VM 5–15V, 출력 VM/5V(1A)/3.3V, 입력 역결선 보호, 채널 2A·피크 2.5A.
...
- **I2C 결선 — 레벨은 측정으로 확정**(Hiwonder 사양이 SDA/SCL 풀업 전압을 명시 안 함):
  - ⚠️ **Pi 전원이 5V여도 GPIO/I2C 로직은 3.3V·5V 비관용**(전원≠로직). SDA/SCL에 5V 인가 시 SoC **영구 손상**.
  - 단서(상충): 보드는 **RPi 5/4B 호환** 표기 + Pi5 사용자가 내장 1.8kΩ(3.3V)로 연결한 사례. 반면 ESP32엔 레벨컨버터 안내도 있음 → 단정 불가.
  - **절차**: VM 인가 후 **SDA/SCL 아이들 전압 측정** → **3.3V면 Pi 직결**(GPIO2/3 내장 1.8kΩ 풀업 사용), **5V면 양방향 레벨시프터(PCA9306/BSS138)**. 불확실하면 시프터로 격리(안전측).
  - 보드 IIC **`5V` 핀은 Pi GPIO에 연결 금지** — 사양표=5V출력 / Q&A=입력으로 **Hiwonder 자료가 상충**하니, 어느 쪽이든 Pi 3.3V 라인에 묶지 말 것(필요 시 레벨시프터 HV 기준으로만).
