...
- **I2C 결선(3선만)**: SDA/SCL/GND만 Pi에 연결. 보드 I2C의 **5V 핀은 출력 → Pi 급전 금지**.
  - ⚠️ **Pi 전원이 5V여도 GPIO/I2C 로직은 3.3V이며 5V 비관용**(전원≠로직). SDA/SCL에 5V가 걸리면 핀과 SoC가 **영구 손상(Pi 사망)** — 과전압 보호 없음. Pi 내장 풀업도 3.3V 기준.
  - 따라서 보드 I2C를 **3.3V 레벨**로 사용: VM 인가 상태에서 **SDA/SCL 아이들 전압 측정 → ≤3.3V면 직결 OK**.
  - **5V로 측정되면 양방향 I2C 레벨시프터(PCA9306/BSS138류) 필수** — 직결 금지.
