...
| 5 | 벨트 모터 — Wheeltec MG310P20 ×2 + 드라이버(Hiwonder 4ch I2C SA8870C, 또는 L298N류) | 컨베이어 구동(시간 기반). 전력상 2채널. **hiwonder는 I2C 5V → 레벨시프터(3.3↔5V) 필요** |
...
```
- **전압 인가(단일 VM)**: 모터 전원만 보드 VM에 인가 → **온보드 레귤레이터**가 VM에서 5V/3.3V(5V 1A)를 만들어 보드 MCU·엔코더를 자체 구동(**별도 로직 전원 불필요**). 모터 전원은 Pi·서보와 **독립**(스톨/기동 전류가 Pi로 흐르면 브라운아웃).
- **I2C 결선 — 이 보드는 5V I2C → RP4B엔 레벨시프터 필수**:
  - ⚠️ **Pi 전원이 5V여도 GPIO/I2C 로직은 3.3V·5V 비관용**(전원≠로직). SDA/SCL에 5V가 걸리면 SoC **영구 손상(Pi 사망)** — 과전압 보호 없음. Pi 내장 풀업도 3.3V.
  - **Hiwonder 공식**: 3.3V MCU(ESP32 등)는 **레벨 컨버터 사용** 안내 → 보드 I2C 버스는 **5V**. 따라서 **양방향 I2C 레벨시프터(PCA9306/BSS138류) 필수** — LV=Pi 3.3V(SDA/SCL+3V3), HV=보드 5V(SDA/SCL+5V). **직결 금지**.
  - 보드 I2C의 **`5V` 핀은 입력(로직 전원, Hiwonder 명시)** — Pi GPIO에 연결 금지(레벨시프터 HV 5V 기준으로만 사용).
  - 확인: VM 인가 후 SDA/SCL 아이들 전압 측정 — 5V면 위 그대로 레벨시프터.
