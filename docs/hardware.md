...
벨트는 **시간 기반**(T_belt초 구동 후 정지, §2.5)이다. 드라이버는 `TRASH_BELT_DRIVER`로 선택한다.
> ⚠️ **실기기 확정(trash-bjl)**: Hiwonder 보드의 **개루프(0x1f)는 이 모터를 못 돌린다** — 보드 MCU가
> 엔코더로 속도를 제어하는 **폐루프 고정속도(0x33)** 만 강하게·안정적으로 구동된다. Pi 드라이버는 폐루프를 쓴다.

**(A) Hiwonder 4채널 엔코더 모터 드라이버 (SA8870C, I2C) — 채택·실기기 검증됨**

> 사양: 칩 **SA8870C**, VM 5–15V, 출력 VM/5V(1A)/3.3V, 입력 역결선 보호, 채널 2A·피크 2.5A.

모터 = Wheeltec **MG310P20** ×1(단일, 포트 **M2** = 채널 `idx1`). 보드 MCU가 엔코더로 속도를 제어하므로
Pi는 **폐루프 고정속도(0x33, −100..100, 부호=방향)** 만 쓴다. 우리 배선은 **음수가 정회전**. 속도 명령은
보드 watchdog로 ~0.3s 후 감쇠하므로 드라이버가 `TRASH_BELT_REFRESH`(~50ms)마다 **재전송**해 연속 회전한다.
구동 전 `MOTOR_TYPE`(0x14=3, JGB37-520)·`ENCODER_POLARITY`(0x15) 초기화 필수(공식 SDK 타이밍 0.5s).
```
Pi GPIO2(SDA)─보드SDA   Pi GPIO3(SCL)─보드SCL   Pi GND─보드GND(공통)   Pi 5V ── 보드 I2C VCC ⚠️5V 필수
보드 VM(모터전원) ── 독립 7.4V(≥5A)              보드 채널 M2 OUT ── MG310P20(7.4V) ×1
```
- **⚠️ I2C 포트 VCC는 반드시 5V**: 보드 I2C 포트 전원핀(VCC)에 **5V** 인가(보드 내부 레귤레이터가 3.3V 생성→MCU 구동). **3.3V 직결 시 MCU 언더파워 → 구동 중 I2C 버스 행(브라운아웃)** — 실기기 확정(trash-bjl, 메모리 `hiwonder-belt-i2c-rootcause`). I2C 신호선은 보드 내부 3.3V라 Pi 직결 안전(첫 배선 시 SDA/SCL 아이들 ≤3.3V 1회 측정 권장). 모터 전원(VM)은 별도 7.4V로 Pi·서보와 **독립**(공통 GND 필수).
