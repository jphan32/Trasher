...
- **화이트밸런스**: AWB auto는 색 물체/조명에 흔들려 흰색이 분홍·적색으로 틀어진다(실측). 부스 조명이
  고정이므로 **수동 ColourGains 고정** 권장(기본 `TRASH_AWB_RED_GAIN=1.6`/`TRASH_AWB_BLUE_GAIN=1.3`).
  현장 재보정: 흰 A4를 화면에 채우고 `ColourGains`를 스윕해 **중앙 R≈G≈B** 되는 값을 찾는다(0=AWB 복귀).
