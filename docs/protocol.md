...
> 씨앗 5종 선정·제공은 **iPad 전용 인터랙션**이라 Pi 상태머신에 없음. `sorting` 완료 후 Pi는 `idle`로 복귀.

### 2.1 사이클 게이팅 (iPad가 다음 사이클 시작을 통제)
Pi는 **"started" 상태일 때만** 변이를 감지한다. iPad가 결과/씨앗 인터랙션 화면에 들어가면 `Command{stop}`, 어트랙트(대기) 화면으로 복귀하면 `Command{start}`를 보낸다. 이렇게 해야 참여자 A의 씨앗 선택 중에 Pi가 B의 투입을 감지하는 화면·물리 흐름 어긋남을 막는다. 즉 **iPad가 인테이크 타이밍의 오케스트레이터**다.
...
- `T_belt`, 서보 각도, 매핑은 모두 Pi 설정값(튜닝 대상).
