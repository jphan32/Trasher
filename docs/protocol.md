...
3개 서보(연속회전 SER0043) + 컨베이어 벨트(모터 ×2)로 구성. 서보=gpiozero(lgpio),
벨트=gpiozero 또는 Hiwonder I2C(택일). 배선·드라이버·전원 상세는 [`hardware.md`](hardware.md).
...
| **분기 서보 우** ×1 | 열림 → 우측 경로 |
| (좌·우 모두 닫힘) | → **중앙 경로** |
| **벨트 모터** ×2 | 컨베이어 구동(시간 기반: 고정 시간 구동 후 완료). 전력상 2채널 |
